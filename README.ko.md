...
`todo.txt`는 일반 텍스트 파일입니다. 우선순위, 프로젝트, 컨텍스트, 생성 및 완료 날짜와 같은 구조화된 작업 메타데이터를 활용하기 위한 몇 가지 간단하지만 유연한 파일 형식 규칙이 있습니다.
...
### 규칙 2: 작업 생성 날짜는 선택적으로 우선순위와 공백 바로 뒤에 표시할 수 있습니다.
...
```
2011-03-02 문서화하기 +TodoTxt 작업 형식에 대해
...
### 규칙 3: 컨텍스트와 프로젝트는 우선순위와 작성일 _뒤_ 라면 아무 곳에나 올 수 있습니다.
...
작업이 소문자 x(대소문자 구분)로 시작하고 바로 뒤에 공백이 온다면, 그 작업은
완료된 것으로 표시됩니다.
...
완료된 작업이 일반적인 정렬 도구를 사용하여 작업 목록의 맨 아래로 정렬되도록 소문자 x를 사용합니다.
...
작업에 등록 일자를 추가한 경우 완료 시 완료 일자 바로 뒤에 표시됩니다. 이렇게 하면 완료된 작업이 일반적인 정렬 도구를 사용하여 날짜순으로 정렬됩니다. 많은 Todo.txt 클라이언트(앱)는 작업 완료 시 우선순위를 버립니다. 만약 우선순위를 남기고 싶다면, 아래에 설명된 `key:value` 형식(예: `pri:A`)을 사용하십시오.
